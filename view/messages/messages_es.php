<?php
  //file: /view/messages/messages_es.php
  // COMPROBAR QUE NO EXISTA LA CLAVE ANTES DE CREAR UNA NUEVA
  $i18n_messages =
  array(
    "User" => "Usuario",
    "Add user" => "Añadir usuario",
    "Full name" => "Nombre completo",
    "Password" => "Contraseña",
    "Add" => "Añadir",
    "Type" => "Tipo",
    "Athlete" => "Deportista",
    "Coach" => "Entrenador",
    "Administrator" => "Administrador",
    "Phone" => "Teléfono",
    "Main page" => "Página principal",
    "Delete" => "Eliminar",
    "Edit" => "Editar",
    "Welcome to SocialFitness!" => "Bienvenido a SocialFitness!",
    "Hello %s" => "Hola %s",
    "User management" => "Administración de usuarios",
    "Edit user" => "Editar usuario",
    "View user" => "Consultar usuario",
    "Edit exercise" => "Editar ejercicio",
    "Easy" => "Fácil",
    "Medium" => "Media",
    "Hard" => "Difícil",
    "Difficulty" => "Difficulty",
    "Details" => "Detalles",
    "Add exercise" => "Añadir ejercicio",
    "Name" => "Nombre",
    "Difficulty" => "Dificultad",
    "User \"%s\" successfully added" => "Usuario \"%s\" añadido satisfactoriamente",
    "User \"%s\" successfully deleted" => "Usuario \"%s\" eliminado satisfactoriamente",
    "User \"%s\" successfully updated" => "Usuario \"%s\" modificado satisfactoriamente",
    "Exercise \"%s\" successfully added" => "Ejercicio \"%s\" añadido satisfactoriamente",
    "Exercise \"%s\" successfully deleted" => "Ejercicio \"%s\" eliminado satisfactoriamente",
    "Exercise \"%s\" successfully updated" => "Ejercicio \"%s\" modificado satisfactoriamente",
    "Machine \"%s\" successfully added" => "Máquina \"%s\" añadida satisfactoriamente",
    "Machine \"%s\" successfully deleted" => "Máquina \"%s\" eliminada satisfactoriamente",
    "Machine \"%s\" successfully updated" => "Máquina \"%s\" modificada satisfactoriamente",
    "Activity \"%s\" successfully added" => "Actividad \"%s\" añadida satisfactoriamente",
    "Activity \"%s\" successfully deleted" => "Actividad \"%s\" eliminada satisfactoriamente",
    "Activity \"%s\" successfully updated" => "Actividad \"%s\" modificada satisfactoriamente",
    "Exercises" => "Ejercicios",
    "Management options" => "Opciones administrativas",
    "Logout" => "Desconectar",
    "Logged as %s" => "Registrado como %s",
    "Machines" => "Máquinas",
    "Activities" => "Actividades",
    "Tables" => "Tablas",
    "Schedule" => "Horario",
    "Capacity" => "Num. plazas",
    "Edit activity" => "Editar actividad",
    "Add activity" => "Añadir actividad",
    "View user" => "Ver usuario",
    "View exercise" => "Ver ejercicio",
    "View activity" => "Ver actividad",
    "Location" => "Ubicación",
    "" => "",
  )
?>